const CartaoOpcoes_render = (function($) {
  ...
  return function(globalProps) {
    ...
    const $opcoes = $('<div>').addClass('opcoesDoCartao');
    ...
    const tipos = Object.keys(globalProps.tipos).map(
      chave => globalProps.tipos[chave],
    );
    ...
    return function(props = {}, state = {}, handlers = {}) {
      ...
      if (ultimaOpcaoNavegada) {
        keyboardNavigation.goTo(elementoNavegacao => {
          if (
            ultimaOpcaoNavegada.attr('class') ===
            elementoNavegacao.attr('class')
          ) {
